...
class GazeBehavExec:
	...
	def __init__(self, config_data, logger):
		...
		self.__gaze_state = GazeBehavior.FOLLOW
	...
	def __gazeBehavThread(self):
		...
		while(True):
			#For debugging
			self.__logger.debug('Current gaze executing %s.' % self.__gaze_state)

			if(self.__gaze_state == GazeBehavior.NEUTRAL):
				self.__gazeNeutral()
			elif(self.__gaze_state == GazeBehavior.FOLLOW):
				self.__gazeFollowing()
			elif(self.__gaze_state == GazeBehavior.AVERSION):
				self.__gazeAversion()
			else:
	...
	def startAverting(self):
		...
		self.__aversion_target_expiry_stamp = -1
		self.__gaze_state = GazeBehavior.AVERSION

	def goToNeutral(self):
		self.__gaze_state = GazeBehavior.NEUTRAL
